...
1. A function, written with a single line of R code, plots the data
...
#+begin_src sh
	 mkdir ~/.emacs.d
#+end_src
1. checkout the latest version of Org-mode into the src subdirectory
...
#+end_src
1. place the following code block in a file called =init.el= in your Emacs
...
#+begin_src emacs-lisp
	   ...
	   ;;; init.el ends here
#+end_src
1. implement all of your Emacs customizations inside of Emacs Lisp
code blocks embedded in Org-mode files in this directory;
and
1. re-start Emacs to load the customizations.
